...
std::vector<std::string> MipsGenerator::generate(const std::string& output_filename) {
    ...
    for (const auto& instr : instructions) {
        ...
        if (instr.name == "ICONST") {
            ...
            assembly_lines.push_back("    li " + reg + ", " + std::to_string(instr.operands[0]) + "\n");
            assembly_lines.push_back("    sw " + reg + ", 0($sp)\n");
            assembly_lines.push_back("    addiu $sp, $sp, -4\n");
            ...
            assembly_lines.push_back("    addiu $sp, $sp, 4\n");
            assembly_lines.push_back("    lw " + reg2 + ", 0($sp)\n"); // Pop second operand
            assembly_lines.push_back("    addiu $sp, $sp, 4\n");
            assembly_lines.push_back("    lw " + reg1 + ", 0($sp)\n"); // Pop first operand
            assembly_lines.push_back("    add " + reg1 + ", " + reg1 + ", " + reg2 + "\n"); // reg1 = reg1 + reg2
            assembly_lines.push_back("    sw " + reg1 + ", 0($sp)\n");   // Push result
            assembly_lines.push_back("    addiu $sp, $sp, -4\n");
